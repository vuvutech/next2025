...
describe("containsMaliciousPatterns", () => {
  ...
  it("detects a <script> tag with attributes", () => {
    expect(
      containsMaliciousPatterns('<script src="evil.js"></script>')
    ).toBe(true);
  });
  ...
  it("returns false for clean text", () => {
    expect(containsMaliciousPatterns("Hello, this is a normal testimonial!")).toBe(false);
  });

  it("returns false for text with numbers and punctuation", () => {
    expect(containsMaliciousPatterns("Great course! Rating: 10/10 (100%).")).toBe(false);
  });
  ...
  it("returns false for URLs without javascript: protocol", () => {
    expect(containsMaliciousPatterns("Visit https://example.com for more info.")).toBe(false);
  });
...
describe("testimonial POST validation pipeline (integration)", () => {
  ...
  function validateTestimonialContent(content: unknown): {
    ...
    if (!validateContentLength(sanitizedContent, 5000)) {
      return { valid: false, error: "Content is too long (maximum 5000 characters)" };
    }

    if (containsMaliciousPatterns(sanitizedContent)) {
      return { valid: false, error: "Content contains invalid characters or patterns" };
    }
  ...
  it("rejects content containing a script tag", () => {
    ...
    const sanitized = sanitizeHtml("Good course <script>alert(1)</script>".trim());
  ...
  it("rejects an onclick event handler in content", () => {
    const result = validateTestimonialContent('Nice course! <div onclick="evil()">click</div>');
    ...
    const sanitized = sanitizeHtml('Nice course! <div onclick="evil()">click</div>'.trim());
    const hasMalicious = containsMaliciousPatterns(sanitized);
