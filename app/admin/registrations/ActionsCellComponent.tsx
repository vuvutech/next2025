          ...
          {loadingUser ? (
            ...
            <Card className="-space-y-5 border-dotted divide-y-1 divide-muted  ">
              <CardHeader>
                <div className="flex items-center gap-4 pb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {userDetails.profile?.telephone || "No telephone specified"}, {userDetails.profile?.mobile || "No mobile phone specified"}
                    </p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="-space-y-1  border-dotted divide-y-1 divide-muted  ">
                ...
                <div className="flex items-center justify-between gap-2 py-2">
                  <span className="font-semibold">Disability Assistance Required:</span>
                  <span className="text-xs text-muted-foreground">
                    {userDetails.profile?.disabilityAssistance ? "Yes" : "No"}
                  </span>
                </div>
                <div className="flex flex-col items-start justify-between gap-2 py-2">
                  <span className="font-semibold">Disability Assistance Type:</span>
                  <span className="text-xs text-muted-foreground">
                    {userDetails.profile?.disabilityDescription || "No disability assistance specified"}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-2 py-2">
                ...
                <div className="flex items-center justify-between gap-2 py-2">
                  <span className="font-semibold">Emergency Contact Telephone:</span>
                  <span className="text-xs text-muted-foreground">
                ...
                <div className="flex items-center justify-between gap-2 py-2">
                  ...
                  <span className="text-xs text-muted-foreground text-right">
                    ...
                    {`${userDetails.profile?.addressLine2 || ""}, `}<br />
                ...
                <div className="flex items-center justify-between gap-2 py-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {userDetails.profile?.facebook || "No Facebook account specified"}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-2 py-2">
                  <span className="font-semibold">Twitter (X):</span>
                  <Link target="_blank" href={userDetails.profile?.twitter || "#"} className="text-xs text-muted-foreground">
                    {userDetails.profile?.twitter || "No Twitter account specified"}
                  </Link>
                </div>
                <div className="flex items-center justify-between gap-2 py-2">
                  <span className="font-semibold">Youtube:</span>
                  <Link target="_blank" href={userDetails.profile?.youtube || "#"} className="text-xs text-muted-foreground">
                    {userDetails.profile?.youtube || "No Youtube account specified"}
                  </Link>
                </div>
                <div className="flex items-center justify-between gap-2 py-2">
                  <span className="font-semibold">LinkedIn:</span>
                  <Link target="_blank" href={userDetails.profile?.linkedin || "#"} className="text-xs text-muted-foreground">
                    {userDetails.profile?.linkedin || "No LinkedIn account specified"}
                  </Link>
                </div>
                <div className="flex items-center justify-between gap-2 py-2">
                  <span className="font-semibold">Instagram:</span>
                  <Link target="_blank" href={userDetails.profile?.instagram || "#"} className="text-xs text-muted-foreground">
                    {userDetails.profile?.instagram || "No Instagram account specified"}
                  </Link>
                </div>
                <div className="flex items-center justify-between gap-2 py-2">
                  <span className="font-semibold">TikTok:</span>
                  <Link target="_blank" href={userDetails.profile?.tiktok || "#"} className="text-xs text-muted-foreground">
                    {userDetails.profile?.tiktok || "No TikTok account specified"}
                  </Link>
                </div>
