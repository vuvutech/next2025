...
describe("PUT /api/users/ban", () => {
	...
	describe("authorization", () => {
		...
		it("allows ADMIN to ban a user", async () => {
			...
			const res = await PUT(
				makeRequest({ id: "user-1", banned: true, banReason: "Violation" }),
			);
	...
	describe("banning a user – session deletion (NEW behavior)", () => {
		...
		it("sets banExpires ~7 days from now when banning", async () => {
			...
			expect(banExpires.getTime()).toBeGreaterThanOrEqual(
				before + sevenDaysMs - 100,
			);
			expect(banExpires.getTime()).toBeLessThanOrEqual(
				after + sevenDaysMs + 100,
			);
		});
		...
		it("returns success:true with the updated user in the response body", async () => {
			...
			const res = await PUT(
				makeRequest({ id: "user-1", banned: true, banReason: "Test ban" }),
			);

			expect((res as unknown as { status: number }).status).toBe(200);
			const body = (
				res as unknown as { body: { success: boolean; user: unknown } }
			).body;
